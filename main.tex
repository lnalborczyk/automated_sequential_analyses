...
Obviously, It is very hard to obtain robust results concerning the effect size of analyst and observer expectancy effects. This "meta-science" problem \lnnote{je pense qu'on peut le dire autremenent, mais je n'ai pas mieux pour le moment...} is complicated because these biases can apply at all the levels as one experiment is included in another \lnnote{pas compris}. It is also difficult to collect large observation sets by experimental conditions as we can note \lnnote{"as we can note in" ou "as noted in" ?} in \cite{zoble_interaction_1969}. Thus, we can only draw attention to these effects as a potential risk to consider rather than as a robust and clearly identified danger to avoid. \par
...
The more we are able to blind ourself in experiment the best it could be (REF). \lnnote{peut-être qu'on pourrait se passer de cette première phrase pour économiser des mots ?}
...
We acknowledge that only a few and non recent papers have highlighted experimenters biases, but we think that we can not take the risk with a so highly rigorous procedure to add any social bias \lnnote{je ne suis pas certain de comprendre la deuxième partie de cette phrase...}. Combining both of those procedures (SBF and triple blinding) could, in our view, increase again the transparency and the rigor of research. With this comment we would like to point out the importance of SBF procedure when it is combined with a blinding procedure, but maybe even more the importance of a global approach of automaticity and blinding \lnnote{what do you mean ?}. In this vein, our proposal is somehow similar to the \textit{born-open data} proposal of \cite{rouder_what_2016}... \lnnote{toujours dans l'idée d'économiser des mots, je proposerais soit de plus détailler les similarités avec rouder 2016, soit de zapper cette partie}
