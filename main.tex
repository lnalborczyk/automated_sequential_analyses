...
\abstract{We discuss the use of the Sequential Bayes Factor (SBF) procedure as introduced by \cite{schonbrodt_sequential_2017} when confronted to real world data. Contrary to simulated data, real world data can be complicated to handle. Several choices have to be made before statistical modeling to ensure subsequent model comparison is sensible. The SBF procedure is expected to inform us about the adequate sample size to reach a conclusion, based on accumulating data. We therefore propose that one should also prepare data in a sequential way before actually computing a Bayes Factor. We propose a full automation procedure, in line with the preregistration philosophy, allowing triple-blinding of analyses. We provide recommendations on how to implement this without additional costs, while taking into account the specificity of the sequential testing situation.}
...
\lnnote{étant donné qu'on veut mettre l'accent sur la première partie du commentaire (automation) plutôt que sur les effets d'epxérimentateur, je pense qu'on pourrait finalement se passer de la figure et se contenter de la table, non ?}
...
\lnnote{Inclure deux trois phrases ici pour présenter l'exemple, cf le script que j'ai partagé sur Mattermost dans lequel je donne un exemple de ce qu'on propose, utilisant les données de: https://osf.io/apidb/. Est-ce qu'on en parle ici, ou pas ?}
