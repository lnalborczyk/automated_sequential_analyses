...
\abnote{je trouve ça lourd deux citations a la suite, je propose : However, testing data a large amount of time with NHST paradigm is not allowed because of the Type I error growth \cite{schonbrodt_sequential_2017}. These authors also add \textit{Of course, one can calculate statistics during data collection, but the results of these tests must not have any influence on optionally stopping data collection}."} \lnnote{Yes, je suis d'accord avec toi, c'est lourd les deux citations de schonbrodt et al à la suite. Par contre je ne suis pas fan de citer schonbrodt et al pour la première partie de la phrase (i.e., l'inflation du taux d'erreur de type 1), ce n'est pas vraiment eux qui ont "découvert" ça. Je suggère que soit on énonce ça comme un fait général et obvious (et on ne cite personne), soit on cite une référence plus consensuelle (mais je ne sais pas qui ?), non ?}
...
\lnnote{Je fais des suggestions ci-dessous pour reformuler cette section en considérant notre discussion du dernier skype}
...
\lnnote{je propose de supprimer la section "conclusion", je trouve que les dernières phrases de la section précédente font déjà une conclusion assez stylée !}
